 ...
 * Script 01
 *
 * Objetivo: comando para salvar os dados como se fossem oriundos de um  sistema de faturamento
 * (ou ERP) que nos seria disponibilizado para a importação na nossa plataforma de BI
 *
 * Resultado: Geração do arquivo MassaDados2005.rpt, com 5.151 linhas para nossa massa de testes
 */
...
select
		...
		case
			when g.FirstName + ' ' + g.LastName = 'Rachel Valdez' then '' -- Ela é a Chefe!
			...
			when g.FirstName + ' ' + g.LastName in ('Shu Ito', 'Stephen Ji ang', 'Syed Abbas') then 'Michael Blythe' -- Esses são funcionarios do Michael.
			when g.FirstName + ' ' + g.LastName in ('Tete Mensa-Annan', 'T svi Reiter') then 'Stephen Jiang' -- Esses são funcionarios do Stephen.
